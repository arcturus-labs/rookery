...
| **Message queueing + send-now steering** | Client-side serialization: pressing Enter always submits; if the agent is busy the message is queued locally. Queued items can be edited, deleted, or sent-now. Send-now uses a separate server/runtime path (`_rookery/steering_prompt` → `SessionRoom.sendSteeringMessage` → runtime-specific handling) so provider-specific behavior stays inside the agent subclass/runtime boundary. `PiAgent` now uses a `pi-acp` extension plus pi `streamingBehavior: "steer"`; the generic ACP fallback remains next-safe-point injection. |
...
- **Stop vs cancel are separate.** `stop()` still tears the runtime down and rejects an in-flight run before `stopImpl()`. `cancel()` is the user-facing "Stop" control in chat: it sends ACP `session/cancel` so the current turn ends cleanly with `stopReason: "cancelled"` while the session stays alive for the next prompt.
...
- **Send-now steering** (`sendSteeringMessage`): bypasses the room's next-turn queue and delegates straight to `agent.sendSteeringMessage()`. The current ACP-backed implementation applies send-now text at the next safe point inside the active workflow before the normal queued-turn drain resumes.
...
`ComposeBox` (`components/ComposeBox.tsx`) is a 4-row textarea; **Enter submits, Shift+Enter inserts newline**. While the agent is working, Enter still queues the typed message automatically, but the primary button flips from **Send** to **Stop** and triggers ACP cancellation instead of queueing.

**Message queueing** (`ChatPanel.tsx:292-451`): `handleSubmit` checks `isAgentProcessingRef`. If the agent is busy, the message is pushed to `queueRef` and shown in a `cwa-queue` list; queued items can be edited, deleted, or **Send now**'d. Send-now removes the item from the normal next-turn FIFO and calls `RemoteAgent.sendSteeringMessage()` (`_rookery/steering_prompt` on the websocket). On `run_completed`, `handleRunCompletion` shifts the next ordinary queued message and fires it after a 120ms timeout, keeping a serialized one-at-a-time run loop. A `cwa-status-line` shows the current `AgentRunStatus` (`idle/busy/thinking/streaming/using_tool/retrying/queued/error`) with a status dot.
